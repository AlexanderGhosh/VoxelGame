...
void Components::FlightControls::processKeys(const std::array<bool, 1024>& keysPressed, const glm::vec3& fwd)
{
	const glm::vec3 right = glm::normalize(glm::cross(fwd, glm::vec3(0, 1, 0)));

	if (keysPressed[GLFW_KEY_W]) {
		_transform->position += fwd * _speed;
	}
	if (keysPressed[GLFW_KEY_S]) {
		_transform->position -= fwd * _speed;
	}
	if (keysPressed[GLFW_KEY_A]) {
		_transform->position -= right * _speed;
	}

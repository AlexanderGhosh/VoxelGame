...
World::World() : created(0), chunks2(), geomDrawable(), seed(), isDynamic(), reDraw(), worldMap() {
}
World::World(bool gen, bool terrain, bool isDynamic, unsigned int seed) : World() {
	this->seed = seed;
...
void World::render(Camera& c, glm::mat4 projection, glm::mat4 lightMatrix, unsigned int depthMap) {
...
std::array<std::vector<glm::vec2>, 4> getNewOld(glm::vec2 oldChunkPos, glm::vec2 newChunkPos) {
	...
	int rd = RENDER_DISTANCE % 2 ? RENDER_DISTANCE : RENDER_DISTANCE + 1;
...
std::tuple<glm::vec3, FACES_NAMES> World::getIntersectedBlock(ChunkColumn*& chunkOcc, Ray ray) {
	...
	std::vector<std::tuple<ChunkColumn*, std::unordered_map<unsigned int, FaceB_p>>> meshes;
	for (auto& chunk : chunks2) {
		meshes.push_back({ &chunk, chunk.getMesh() });
	}
	
	auto isSameDirection = [](glm::vec3 pos, Ray ray) -> bool {
	...
	for (auto& mesh : meshes) {
		...
		std::unordered_map<unsigned int, FaceB_p>& faces = std::get<1>(mesh);
...
std::vector<ChunkColumn*> World::getAdjacentChunks(glm::vec3 worldPosition)
{
	...
	for (glm::vec2& pos : chunksToFind) {
		for (ChunkColumn& column : chunks2) {
...
AdjacentMap World::getAdjacentMap(glm::vec3 worldPos, unsigned int range)
{
	AdjacentMap res;
	...
	for (glm::vec2& pos : poss) {
		...
		res.emplace(pos, ChunkColumn(pos, seed));
	}
...
AdjacentMap_p World::getAdjacentMapPointers(glm::vec3 worldPos, unsigned int range)
{
	AdjacentMap_p res;
...
std::vector<glm::vec2> World::centeredPositions(glm::vec2 origin, std::vector<glm::vec2> exclude, int renderDist) {
...
std::vector<glm::vec2> World::centeredPositions(glm::vec2 origin, std::vector<ChunkColumn>& exclude, int renderDist)
{
	origin.x = reduceToMultiple(origin.x, CHUNK_SIZE);
	origin.y = reduceToMultiple(origin.y, CHUNK_SIZE);
	if (renderDist % 2 == 0)  renderDist++;
	...
	for (GLbyte x = -radius; x < radius + 1; x++) {
		for (GLbyte y = -radius; y < radius + 1; y++) {
			...
			pos += origin;
			bool found = false;
			for (const ChunkColumn& column : exclude) {
				if (column.getPosition() == pos) {
					found = true;
					break;
				}
